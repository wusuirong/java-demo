...
public class A0011_KafkaProducer异步发送Demo {
	...
	public static void main(String[] args) throws InterruptedException, ExecutionException {
		...
		props.put(ProducerConfig.BATCH_SIZE_CONFIG, 300); // buffer中所有record占用字节数超过此配置时，就要发送，好比公交车上满了就可以发车了，不用等到发车时间。
		props.put(ProducerConfig.LINGER_MS_CONFIG, 500); // 延迟发送时间，当buffer存入record后，如果超过此时间没有新record加入，则发送buffer的record，避免buffer一直不满导致record不发送。好比公交车到点就要发车，即使没坐满。
		...
		for (int i = 0; i < 100; i++) {
			...
			Future<RecordMetadata> future = producer.send(record, new Callback() {
				...
				public void onCompletion(RecordMetadata metadata, Exception exception) {
					// 注意这里是kafka的发送线程回调的，所以不要做太多耗时操作，否则会拖慢发送线程。
					System.out.printf("record到达server，topic: %s, partition: %d, offset: %d\n", metadata.topic(), metadata.partition(), metadata.offset());
				}
				
			});
			
			System.out.println("异步发送数据" + i);
			Thread.sleep(100);
		}
