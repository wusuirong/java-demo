...
public class OrderListTest extends TestCase {
	...
	//测试基于连续空间时数组重分配的情况
	public void testResizeList() throws IndexOutOfBoundsException {
		List list = this.createList();
		list = this.addHumans(5, list);
		...
		list = this.addHumans(1, list);
		this.assertEquals(11, list.length());
		this.assertEquals(12, list.size());
		
		list = this.addHumans(2, list);
		this.assertEquals(13, list.length());
		this.assertEquals(14, list.size());
		
		Human e = list.get(10);
		this.assertEquals(10, e.id);
		
		e = list.get(11);
		this.assertEquals(11, e.id);
		
		e = list.get(13);
		this.assertEquals(13, e.id);
