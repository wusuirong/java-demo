...
public class VolatileObjectTest2 {
	...
	boolean boolValue = true;// 加上volatile 修饰的是时候，程序会很快退出，因为volatile
						// 保证各个线程工作内存的变量值和主存一致。所以boolValue == !boolValue就成为了可能。

	public void waitToExit() {
		if (boolValue == false)
			System.exit(0);// 非原子操作，理论上应该很快会被打断。实际不是，因为此时的boolValue在线程自己内部的工作内存的拷贝，因为它不会强制和主存区域同步，线程2修改了boolValue很少有机会传递到线程一的工作内存中。所以造成了假的“原子现象”。
	}
