...
public class HttpProxyServer implements Runnable, LogMessage {
	private int port;
	...
	"txt", "text/plain", "html", mime_text_html, "htm", "text/html", "gif", "image/gif", "jpg", "image/jpg", "jpeg", "image/jpg", "class", "application/octet-stream" };
	...
	private void logEntry(String cmd, String url, int code, int size) {
		...
		int tzhour = tzmin / 60;
		tzmin -= tzhour * 60;
		log.log(host + " - - [" + fmt02d(calendar.get(Calendar.DATE)) + "/" + months[calendar.get(Calendar.MONTH)] + "/" + calendar.get(Calendar.YEAR) + ":" + fmt02d(calendar.get(Calendar.HOUR))
				+ ":" + fmt02d(calendar.get(Calendar.MINUTE)) + ":" + fmt02d(calendar.get(Calendar.SECOND)) + " " + fmt02d(tzhour) + fmt02d(tzmin) + "] \"" + cmd + " " + url + " HTTP/1.0\" " + code
				+ " " + size + "\n");
		hits_served++;
		bytes_served += size;
	}
	...
	private void handleProxy(OutputStream out, String url, MimeHeader inmh) {
		try {
			...
			if (!serveFromCache(out, url)) {
				...
				inmh.put("User-Agent", inmh.get("User-Agent") + " via JavaCompleteReferenceProxy/" + version);
	...
	public HttpProxyServer(int p, String dr, LogMessage lm) {
		port = p;
		docRoot = dr;
		log = lm;
	}
	...
	public static void main(String args[]) {
		HttpProxyServer h = new HttpProxyServer(80, "c:\\www", null);
		h.log = h;
		h.start();
		try {
