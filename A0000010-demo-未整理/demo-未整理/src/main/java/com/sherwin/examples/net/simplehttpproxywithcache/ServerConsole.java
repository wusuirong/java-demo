...
public class ServerConsole extends Applet implements LogMessage, ActionListener {
	private int m_port = 80;
	private String m_docroot = "c:\\www";
	private HttpProxyServer m_httpd;
	...
	public void init() {
		...
		p.add(status);
		m_httpd = new HttpProxyServer(m_port, m_docroot, this);
	}
	...
	public void log(String msg) {
		m_log.append(msg);
		status.setText(m_httpd.hits_served + " hits (" + (m_httpd.bytes_served / 1024) + "K), " + m_httpd.files_in_cache + " cached files (" + (m_httpd.bytes_in_cache / 1024) + "K), "
